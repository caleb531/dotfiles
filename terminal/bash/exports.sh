...
export HISTIGNORE='clear*:cprmlastcmd*:git branch -d *:git branch -D *:git checkout *:git clean*:git reset *:git rm*:git stash drop*:history*:kill *:killall *:reload*:rm *:rmlastcmd*:sudo *'
